...
export default function EditProduct() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: cats }, { data: product }, { data: imgs }, { data: vars }] = await Promise.all([
      ...
      if (product) {
        ...
        setMaterialDetails(product.material_details || '')
  ...
  const addImage = async () => {
    ...
    const { data } = await supabase
      .from('product_images')
      .insert({ product_id: id, image_url: newImageUrl, display_order: images.length })
      .select()
      .single()
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <form onSubmit={handleSubmit} className={formStyles.form}>
        ...
        <button type="button" onClick={handleDelete} className={formStyles.deleteBtn}>
          Delete Product
        </button>
      </form>
      ...
      <div className={formStyles.section}>
        ...
        <div className={formStyles.addRow}>
          ...
          />
          <button onClick={addVariant} type="button">Add Variant</button>
        </div>
      </div>
    </div>
  )
